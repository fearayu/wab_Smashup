...
form.addEventListener('submit', async event => { event.preventDefault(); const username = document.querySelector('#username').value.trim(); const email = document.querySelector('#email').value.trim().toLowerCase(); const password = document.querySelector('#password').value; if (!username || password.length < 6 || (registerMode && !email)) { setFeedback('กรุณากรอกข้อมูลให้ครบ และรหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร', 'error'); return; } const users = getUsers(); const passwordHash = await hashPassword(password); if (registerMode) { if (users.some(user => user.username === username || user.email === email)) { setFeedback('username หรืออีเมลนี้มีบัญชีอยู่แล้ว', 'error'); return; } const user = { id: crypto.randomUUID(), username, name: username, email, passwordHash, createdAt: new Date().toISOString() }; users.push(user); localStorage.setItem(USERS_KEY, JSON.stringify(users)); localStorage.setItem(SESSION_KEY, JSON.stringify({ id: user.id, name: user.username, email: user.email })); setFeedback('สร้างบัญชีเรียบร้อย กำลังพาไปหน้าแรก…', 'success'); } else { const user = users.find(item => (item.username === username || (!item.username && item.email === username)) && item.passwordHash === passwordHash); if (!user) { setFeedback('username หรือรหัสผ่านไม่ถูกต้อง', 'error'); return; } localStorage.setItem(SESSION_KEY, JSON.stringify({ id: user.id, name: user.username || user.name || user.email, email: user.email })); setFeedback('เข้าสู่ระบบสำเร็จ กำลังพาไปต่อ…', 'success'); } setTimeout(() => { location.href = nextPage(); }, 500); });
renderMode();
